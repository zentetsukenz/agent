...
export async function handleRequest(
  ...
  if (request.command) {
    switch (request.command) {
      case "plan":
        ...
        stream.markdown(
          `**Goal**: ${request.prompt || "No goal specified"}\n\n`
        );
        stream.markdown(
          "*Planning mode active. I will research before implementing.*\n"
        );
      ...
      case "implement":
        ...
        stream.markdown(
          "*Implementation mode active. I will make changes with checkpoints.*\n"
        );
      ...
      case "checkpoint":
        ...
        stream.markdown(
          `*Saving checkpoint: ${request.prompt || "Manual checkpoint"}*\n`
        );
  ...
  stream.markdown(
    "*Use `/plan`, `/implement`, or `/checkpoint` for workflow commands.*\n"
  );
