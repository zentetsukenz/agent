...
- 2026-07-31 — Added OpenCode (primary/subagent agents, commands, skills, `permission:`-based tool control, `AGENTS.md`/`instructions:` context, the two GAPs — no memory tool, no description-triggered instructions — and an opt-in central model-tiering layer, later dropped by ADR-021); foundation reference for the loom-OpenCode adapter (`adapters/opencode/setup.md`, ADR-014)
- 2026-08-17 — Added Domain Docs: the canonical home for glossary/ADR **path resolution** (`loom.toml#paths.*` → `wiki/` → legacy `CONTEXT.md`/`docs/adr/`) and single/multi-context **file structure**, extracted from the duplicated preambles in `grill-with-docs`, `domain-model`, and `improve-codebase-architecture` (link, don't embed); mirrors the Issue Tracker environment pattern
