...
> adapter supplies only Mirai's **five port answers** (see
...
## Mirai's five port answers (the adapter's job)

Everything Mirai-specific is one of the five [port obligations](../../contract/PORTS.md):
...
| **`mechanism→install`** | A repo-local tree: the Mechanism-layer distributable lands at `.mirai/mechanisms/loom` (executable, no extension) inside the project's own `.mirai/`, alongside `agents/`/`prompts/`/`skills/`/`instructions/`. Invoked by that direct repo-relative path (Mirai has no PATH/alias primitive to lean on) — `.mirai/mechanisms/loom size score`. Two callers share this one installed path: Planning's task-sizing and the Orchestrator's agent-selection rubric ([implementation.md](../../workflows/sdlc/implementation.md#the-orchestrator)). See [Port 5](../../contract/PORTS.md#port-5--mechanisminstall) for the obligation; step 6 below performs the placement. |
...
6. **Install the Mechanism-layer distributable** — place it at `.mirai/mechanisms/loom` (create
   the directory if absent; `chmod +x`) per the [`mechanism→install`](#mirais-five-port-answers-the-adapters-job)
...
7. **Verify** — run the generic invariant-checks
   ...
   ([references/verify.md](references/verify.md)), confirming `.mirai/mechanisms/loom` exists and is executable.
8. **Done** — report created vs. patched paths (including `.mirai/mechanisms/loom`) and flag
   anything deferred.
...
- A `.mirai/` tree: `agents/`, `prompts/`, `skills/<slug>/`, `instructions/` (always includes
  `handoff.instructions.md`), `mechanisms/loom` (the installed distributable), plus root
  `AGENTS.md` (or `.mirai/mirai-instructions.md` — never both).
- A seeded ledger manifest at the chosen `<ledger-root>/index.md`.
- A short report of created vs. patched paths, including whether `.mirai/mechanisms/loom` was
  created or patched.
