...
| Question | Recommended default | Signal to deviate |
|---|---|---|
| Where should the seam-artifact **ledger** live? | A **gitignored on-disk folder** — the ledger is *ephemeral coordination*, not version-controlled ([ADR-014](../wiki/adr/adr-014-loom-opencode-setup.md) Option A); durable knowledge belongs in the wiki/ADRs, not the handoff trail. If the harness has memory, add a lightweight manifest pointer in memory for fast agent discovery. | Team explicitly wants reviewable handoff diffs in PRs → opt to **commit** the folder (accepting state files in the code tree); harness has no memory tool → on-disk folder only (no memory pointer). **Note:** if this project's SDLC runs are dispatched into by a *resident* macro agent on a different harness (see [harness-archetypes](../wiki/patterns/harness-archetypes.md)), the on-disk folder is **mandatory and always gitignored** — memory cannot cross a harness boundary. |
| **Ledger root** path? | Gitignored on-disk: `.loom/handoffs/` (+ a memory root if the harness has memory) | Project already namespaces tooling under a different dir (e.g. `.tooling/`) — match it |
