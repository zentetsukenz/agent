...
| Question | Recommended default | Signal to deviate |
|---|---|---|
| Where should the seam-artifact **ledger** live? | A **local-only, blanket-gitignored on-disk folder** — the ledger is *ephemeral coordination*, not version-controlled ([ADR-014](../wiki/adr/adr-014-loom-opencode-setup.md)); the whole `.loom` seam (protocol document, manifest, and artifacts) is ignored and **no `.loom` path is committed or tracked**. Durable knowledge belongs in the wiki/ADRs; durable/reviewable artifacts belong on the reachable orphan-ref substrate ([ADR-022](../wiki/adr/adr-022-reachable-artifact-substrate.md)), not in a Git-visible `.loom`. If the harness has memory, add a lightweight manifest pointer in memory for fast agent discovery. | Harness has no memory tool → on-disk folder only (no memory pointer). **On `update`, preserve an existing blanket `.loom` ignore — never propose selective rules that would track the protocol/manifest.** **Note:** if this project's SDLC runs are dispatched into by a *resident* macro agent on a different harness (see [harness-archetypes](../wiki/patterns/harness-archetypes.md)), the on-disk folder is **mandatory** — memory cannot cross a harness boundary. |
| **Ledger root** path? | Blanket-gitignored on-disk: `.loom/handoffs/` under a `.loom/**` ignore (+ a memory root if the harness has memory) | Project already namespaces tooling under a different dir (e.g. `.tooling/`) — match it |
