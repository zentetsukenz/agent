...
const { validateEndpointData } = require(path.join(__dirname, '../../../src/features/endpoints/endpoints.service'));
...
describe('Endpoints Service - Unit Tests', () => {
  describe('validateEndpointData', () => {
    ...
    test('should validate URL format', () => {
      ...
      const result = validateEndpointData(invalidData);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('URL must be valid (http:// or https://)');
    });
    ...
    test('should validate method is one of allowed values', () => {
      ...
      const result = validateEndpointData(invalidData);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Method must be one of: GET, POST, PUT, DELETE, PATCH');
    });
