...
- **The Core/Gate/Mechanism layer model** — implements `wiki/adr/adr-026-gate-mechanism-layer-model.md`. Three months of use refuted `wiki/adr/adr-002-workflow-as-adapter-seed.md`'s bet: of 28 SDLC exit-gate criteria only one named an executable command, and that one (`scripts/validate.sh`) was itself a silent no-op (root resolved one level too high, globbed a lowercase-typo'd path — reported `0 validated … all OK`, exit 0, on a renamed working-tree copy). loom is now organised into three layers, each owning one kind of slop, each enforced by a mechanism suited to what it owns: **Core** (prose: skills/wiki/ADRs) owns judgment, enforced by reasoning plus a different actor; **Gate** (a committed per-project typed-criteria file) owns form, enforced by exit codes and withheld capabilities; **Mechanism** (scripts, skill-local → shared → structural) owns recall, enforced by re-executing reality. Supersedes exactly two sentences of ADR-002 (its status flips to `Superseded by ADR-026`; body and historical record stay intact) — prose-first-for-judgment is **not** reversed, and executable code still never lives in the workflow seed. Grants `contract/PORTS.md` one narrow, committed exception to "no schema, no DSL": the new root `GATE.md`.
  - **`GATE.md`** — new root file, 27 typed exit criteria (8 `artifact` / 18 `judgment` / 1 `executable`) replacing self-attested phase exit gates.
  - **`REGISTRY.md`** — new root file, the producer↔mechanism join for every `executable`/`artifact` criterion that names a Mechanism.
  - **`scripts/validate.sh`** — root-resolution bug fixed (`$SCRIPT_DIR/../..` → `$SCRIPT_DIR/..`), the resulting double `agent/` prefix dropped, `SKILLS` casing corrected; `skills`/`wiki`/`links` classes now fail closed unconditionally on a zero count (no directory guard), while `.mirai`/`.opencode`/`.hermes` skip when genuinely absent but fail when present-and-empty; gained 4 new check classes (`docs/` link coverage, anchor validation, orphan detection, REGISTRY↔GATE join).
  - **`SKILLS/planning/task-sizing/SKILL.md`** — re-banded `Small`/`Medium`/`Large` to `5-6`/`7-8`/`9+` (the old `1-3` band was mathematically unreachable — 5 dimensions × minimum 1 ⇒ minimum score 5); worked-example typo `~8%`→`~9%` fixed; `scripts/size-score.sh` added with a `--self-test` proving coverage/disjointness/reachability/no-straddle over the real domain.
  - `wiki/patterns/ratchet.md` — new pattern routing a Preservation lesson by slop kind (judgment→prose, form→a `GATE.md` row, recall→a Mechanism, ratcheted on **first** occurrence); `workflows/sdlc/preservation.md` feedback loop rewired to the three destinations.
  - 10 new glossary terms (Form/Recall/Judgment slop, Gate, Evidence producer, Criterion type, Mechanism, Graduation, Ratchet, Re-executing reality).
- **The `mechanism→install` port** — implements `wiki/adr/adr-027-mechanism-install-port.md`. ADR-026 named a fifth port but did not draw it — no adapter's `setup.md` placed an executable distributable on disk in invocable form. Adds **Port 5 — `mechanism→install`** to `contract/PORTS.md` (obligating each adapter to state where the Mechanism-layer distributable lives, how it becomes invocable, and what "installed" means); every "four ports" reference updated to five, including `SPEC.md`'s normative conformance rules and `index.md`. All three adapters answer Port 5 (Mirai/OpenCode: repo-local `.{mirai,opencode}/mechanisms/loom`; Hermes: profile-distribution `<profile>/mechanisms/loom`).
- **The Verifier is reachable from Shaping** — implements `wiki/adr/adr-028-verifier-reachable-from-shaping.md`. `GATE.md` typed 11 Shaping-side judgment criteria as producer `verifier`, but the Verifier's role definition enumerated only Delivery-side consumers (Orchestrator) — a committed row naming an unreachable producer. Shaping is added as a third named consumer of the existing cross-stage Verifier utility (alongside the Orchestrator and the future plan-reviewer) — Shaping already holds `delegate` as a read-only Dispatcher (ADR-021), and the Verifier was already deliberately `dispatched`, not Delivery-scoped. Widens a reuse enumeration only — no new capability, no new actor, no `GATE.md` change; producer ≠ judge stays intact.
- **A deterministic board-API distributable for the macro tick** (`Proposed`) — `wiki/adr/adr-025-deterministic-board-api.md`. macro-PM's tick was specified mechanical/restart-safe but board *mutation* was performed by model reasoning, so the board drifted and determinism was aspirational rather than executable. Decision: board mutation runs through a small `gh`-backed distributable (verbs `read`/`tree`/`apply`/`heal`/`reconcile`, JSON out, nonzero on drift) the tick prose calls at fixed steps (**Heal → Read → Route → Act → Integrate → Reconcile → Exit**); the framework holds only the CLI contract as illustrative pseudo-code — no executable code in the seed. The board is **plain GitHub Issues, not Projects v2** — native issue dependencies (`blockedBy`/`blocking`, no depth limit) carry the graph, and `board tree` renders it as both an indented tree and a native Mermaid `graph TD` block. GitHub sub-issues (capped at 7 levels) are demoted to optional shallow grouping only. Still `Proposed`, blocking the board-API sub-map from leaving `Proposed`.
- **`scripts/loom/` — loom's first distributable, and its first JavaScript.** Implements ADR-025's CLI contract, corrected by `wiki/adr/adr-029-single-edge-board-graph.md` (single-edge board graph — a ticket `blocks` its map exactly as it `blocks` another ticket; membership vs. ordering read off the far node's type, no `parent`/`subIssues`; the corrected five-conjunct `takeable` — `member ∧ type ≠ map ∧ open ∧ no open blocker ∧ unassigned` — fixing a live-board defect where the old three-conjunct predicate over-collected to 9 items including a `wayfinder:map` issue). Zero dependencies, no `package.json`, `node:test` only: `board/model.js` (pure predicates + plan functions, no I/O), `board/render.js` (tree/Mermaid rendering), `board/gh.js` (the **sole** `child_process` site — `execFileSync` with an argv array, bodies via `--body-file -` stdin, never `shell: true`), `board/verbs.js` (the four verbs), `index.js` (the **sole** `console.log`/`process.exit` site). The four verbs — `read`, `tree`, `apply`, `reconcile` — return `{ok, payload, exitCode}` and never print or exit themselves. `reconcile` migrates the legacy `parent` edge to `blocking` (10 edges migrated on the live board, flipping the frontier `[]` → `[20]`) and was independently cross-checked with `unmigrated: 0` after a re-run reporting `10 → 0` repairs. This is loom's first shipped code, so it also establishes this repo's **quality baseline** (ADR-017) for the record: lint = `node --check`, coverage = `node --test` — currently 96 tests, 96 pass, 0 fail. Commits: `ad5458d` (ADR-029), `acef140` (model), `95e04dd` (CLI).

### Fixed

- Corrected a stale research claim propagated from issue #18 into #17/#19/#20 ("no `gh issue create --parent` flag") — `gh` 2.98.0 supports `--parent`/`--add-blocked-by`/`--add-blocking` natively; the `addSubIssue` GraphQL workaround those issues cited is dead.
...
- **`explore` gains a read-only shell so dispatched recon can run `gh`/`git` reads** — fixes issue [#12](https://github.com/zentetsukenz/agent/issues/12), implements `wiki/adr/adr-023-explore-read-only-shell.md`. The `explore` Utility's capability set was `read`, `search` only — **no `shell`** — so the agent ADR-021 routes recon to could not run `gh issue view`, `git log/diff/blame`, or project read commands (`mix test`, `cat`, `ls`), bouncing the work back to the expensive Shaping agent and defeating the delegation contract.
  - **`contract/primitives.md`** — the `explore` capability row **gains `shell`** (`read`, `search`, `shell`); `edit`/`delegate`/`persist` stay withheld. The read-only guarantee is the **withheld `edit`**, not the absence of shell — mirrors the existing `verifier` (`read`, `search`, `shell`, `persist` — no `edit`) and `visual-qa` (`read`, `search`, `shell` — no `edit`) precedent.
  - **`adapters/opencode/STAGES.md`** + **`references/verify.md`** — the emitted `.opencode/agents/explore.md` renders `permission: { edit: deny, bash: allow }`; verify checklist gains the assertion. `adapters/mirai/STAGES.md` inherits via the generic contract (Mirai lists the `execute` tool, `edit` unlisted — withhold-by-omission).
  - **No new port, no new primitive** — resolves through the existing `capability→tool` port. Rejected: documenting the limit + routing to `quick`/`deep` (defeats ADR-021's cost win), per-command `bash` allow-listing (fragile/harness-specific), a new read-only-shell capability variant (over-engineered). Supersedes ADR-021's "`explore` is `read`+`search` only" render note (ADR-021 body left immutable). Registered in `wiki/adr/{index,log}.md`, CHANGELOG, root `log.md`.
...
- **Opt-in central model-tiering layer dropped from the OpenCode adapter** — amends `wiki/adr/adr-014-loom-opencode-setup.md` (via ADR-021). loom already expresses per-role tiering via inline `model:` on each agent/command, so the opt-in central-tiering overlay was a second way to do the same thing. **Deleted** `adapters/opencode/references/omo.md`; removed its opt-in interview question, write-format section + placeholder notes, verify branch, and mentions in `setup.md`, `MAPPING.md §5`, `wiki/environments/opencode.md`, `SPEC.md`, and `SETUP.md`. `archetype→model` is now inline `model:` only.
