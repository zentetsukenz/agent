...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or UI/logic code via the [prototype](../../implementation/prototype/SKILL.md) skill. Publishes the prototype (or its kept answer) to the reachable [artifact ref](../../../wiki/glossary/index.md#artifact-ref) and links its URL from the ticket (the [PRODUCE](#work-through-the-map) step), so a later session or dispatched run can reach it. Use when "how should it look" or "how should it behave" is the key question.
- **Grilling** (HITL): Conversation via the [grill-with-docs](../../discovery/grill-with-docs/SKILL.md) skill, one question at a time. The default case. Any design doc/findings it produces publish to the reachable [artifact ref](../../../wiki/glossary/index.md#artifact-ref) and link from the ticket (the [PRODUCE](#work-through-the-map) step) — never a local `.loom/` path.
...
> **PRODUCE — link a reachable artifact, never a local path** ([ADR-022](../../../wiki/adr/adr-022-reachable-artifact-substrate.md)). Resolving a ticket may create content a later reader needs — a `grilling` design doc, a `prototype`'s output, `research` findings. This step is **uniform and unconditional**: persist whatever the resolution produced to the project's **reachable [artifact ref](../../../wiki/glossary/index.md#artifact-ref)** via [`persist`](../../../wiki/patterns/role-scoped-capabilities.md) and link the returned **URL** from the ticket. It is a **no-op when nothing was produced** — so it never becomes an `if ticket.type` branch. It reuses the [stage-handoff](../../preservation/stage-handoff/SKILL.md) PRODUCE contract (write + return a reachable pointer) rather than restating it. A link to a local-only path (`.loom/…`, harness memory) is a [reachability-invariant](../../../wiki/patterns/seam-artifact-protocol.md#the-macro-section-and-the-one-source-of-truth-invariant) **violation**: the next reader — especially an SDLC run dispatched across a harness boundary from a `task` leaf pointing here — cannot follow it.
