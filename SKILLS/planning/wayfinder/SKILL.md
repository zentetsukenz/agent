...
> **Macro mode** ([below](#macro-mode-dispatching-into-sdlc-runs)) is exactly that override, formalized: a `task` ticket's execution is not done in the map session — it is **dispatched into an SDLC run** across the [altitude seam](../../../CONTEXT.md#altitude-seam). The map stays a planner; the building happens one altitude down. Absent macro mode, this section stands unchanged.
...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or UI/logic code via the [prototype](../../implementation/prototype/SKILL.md) skill. Publishes the prototype (or its kept answer) to the reachable [artifact ref](../../../CONTEXT.md#artifact-ref) and links its URL from the ticket (the [PRODUCE](#work-through-the-map) step), so a later session or dispatched run can reach it. Use when "how should it look" or "how should it behave" is the key question.
- **Grilling** (HITL): Conversation via the [grill-with-docs](../../discovery/grill-with-docs/SKILL.md) skill, one question at a time. The default case. Any design doc/findings it produces publish to the reachable [artifact ref](../../../CONTEXT.md#artifact-ref) and link from the ticket (the [PRODUCE](#work-through-the-map) step) — never a local `.loom/` path.
...
4. Record the resolution: post the answer as a **resolution comment**, **PRODUCE any artifact this resolution created** (see below), **close** the issue, and **append a context pointer** to the map's Decisions-so-far.
5. Add newly-surfaced tickets (create-then-wire); graduate any fog the answer has made specifiable, clearing each graduated patch from **Not yet specified** so it lives only as its new ticket. If the answer reveals a ticket — this one or another — sits beyond the destination, **rule it out of scope** rather than resolving it on the route. If the decision invalidates other parts of the map, update or delete those tickets.

> **PRODUCE — link a reachable artifact, never a local path** ([ADR-022](../../../wiki/adr/adr-022-reachable-artifact-substrate.md)). Resolving a ticket may create content a later reader needs — a `grilling` design doc, a `prototype`'s output, `research` findings. This step is **uniform and unconditional**: persist whatever the resolution produced to the project's **reachable [artifact ref](../../../CONTEXT.md#artifact-ref)** via [`persist`](../../../wiki/patterns/role-scoped-capabilities.md) and link the returned **URL** from the ticket. It is a **no-op when nothing was produced** — so it never becomes an `if ticket.type` branch. It reuses the [stage-handoff](../../preservation/stage-handoff/SKILL.md) PRODUCE contract (write + return a reachable pointer) rather than restating it. A link to a local-only path (`.loom/…`, harness memory) is a [reachability-invariant](../../../wiki/patterns/seam-artifact-protocol.md#the-macro-section-and-the-one-source-of-truth-invariant) **violation**: the next reader — especially an SDLC run dispatched across a harness boundary from a `task` leaf pointing here — cannot follow it.

The user may run unblocked tickets in parallel, so expect other sessions to be editing the tracker concurrently.

## Macro mode: dispatching into SDLC runs

Everything above is wayfinding at the **macro [altitude](../../../CONTEXT.md#altitude)** — charting effort too big for one [SDLC](../../../workflows/sdlc/index.md) run. **Macro mode** turns the map from a pure planner into the layer that _drives_ the build: its buildable leaves are dispatched **down** into SDLC runs, and those runs report **up** to the map. It is the [Plan, don't do](#plan-dont-do) override made concrete, and it is governed by [ADR-018](../../../wiki/adr/adr-018-macro-project-management.md). Absent macro mode, ignore this section — wayfinder plans and hands off, unchanged.

Macro mode is **opt-in per project, never per map**: it is on exactly when the project's [communication protocol document](../../../wiki/patterns/seam-artifact-protocol.md#the-macro-section-and-the-one-source-of-truth-invariant) names this tracker as the macro **source of truth**. A board either has a resident macro-PM driving it or it does not, so the declaration belongs to the project, not to each effort — a per-map switch would have to be read out of issue prose, which [ADR-029](../../../wiki/adr/adr-029-single-edge-board-graph.md) D3 forbids. Every map on such a board obeys the **one-source-of-truth invariant**: do not open a second, unregistered tracker (a local `TODO.md`, an off-board list) for state that belongs on the map.
...
The map talks to SDLC runs through two label vocabularies on the tracker — the whole [altitude seam](../../../CONTEXT.md#altitude-seam), no side channel:
...
| `wayfinder:task` (buildable leaf)            | an **SDLC run**                                                   | write the ticket + its linked artifacts as a `shaping/<milestone>/` [seam artifact](../../../CONTEXT.md#seam-artifact) in the run's memory ledger, which Planning's DISCOVER gate expects |

**Down** (dispatch a leaf): translate the ticket into the SDLC run's entry artifact, set the leaf `sdlc:in-progress`, and let the run proceed in _its own_ (memory) substrate — the map does not follow it into the inner loop. The leaf's linked artifacts travel as **reachable URLs** into the [artifact ref](../../../CONTEXT.md#artifact-ref) ([ADR-022](../../../wiki/adr/adr-022-reachable-artifact-substrate.md)) — the `shaping/<milestone>/` seam artifact carries the link and the run **fetches only what it needs on demand**, never checking a folder into its working tree. A leaf that links a local-only path (`.loom/…`) is a [reachability](../../../wiki/patterns/seam-artifact-protocol.md#the-macro-section-and-the-one-source-of-truth-invariant) breach: the run, in a different harness, cannot follow it. If the leaf is still fuzzy (a `research`/`grilling` ticket), it is **not** a buildable leaf yet; resolve it as a decision first — a leaf only dispatches to a build once it is decided and specified.
